...
public abstract class BaseTransformDialog extends Dialog implements ITransformDialog {
   ...
   * @return The transform name text field (wTransformName) to use as the first control for layout
   *     chaining (lastControl)
   */
  protected Control createShell(String title) {
    ...
    // Do not add shell image on macOS it overrides the default image in the sidebar
  ...
  public static class ButtonBarBuilder {
    ...
    public ButtonBarBuilder ok(Listener listener) {
      dialog.wOk = createButton("System.Button.OK", listener);
    ...
    public ButtonBarBuilder preview(Listener listener) {
      dialog.wPreview = createButton("System.Button.Preview", listener);
    ...
    public ButtonBarBuilder get(Listener listener) {
      dialog.wGet = createButton("System.Button.GetFields", listener);
    ...
    public ButtonBarBuilder sql(Listener listener) {
      dialog.wSql = createButton("System.Button.ShowEditSQL", listener);
      buttons.add(dialog.wSql);
      return this;
    }

    /**
     * Adds a Create Table button with the specified listener.
     *
     * @param listener the listener to invoke when the button is clicked
     * @return this builder for method chaining
     */
    public ButtonBarBuilder create(Listener listener) {
      dialog.wCreate = createButton("System.Button.CreateTable", listener);
      buttons.add(dialog.wCreate);
      return this;
    }
    ...
    public ButtonBarBuilder cancel(Listener listener) {
      dialog.wCancel = createButton("System.Button.Cancel", listener);
      buttons.add(dialog.wCancel);
      return this;
    }

    /**
     * Adds a custom button with the specified i18n key and listener.
     *
     * @param i18nKey the internationalization key for the button label
     * @param listener the listener to invoke when the button is clicked
     * @return the created button
     */
    public ButtonBarBuilder custom(String i18nKey, Listener listener) {
      Button button = createButton(i18nKey, listener);
      buttons.add(button);
      return this;
    }

    /**
     * Creates a button with the specified i18n key and listener.
     *
     * @param i18nKey the internationalization key for the button label
     * @param listener the listener to invoke when the button is clicked
     * @return the created button
     */
    private Button createButton(String i18nKey, Listener listener) {
      Button button = new Button(dialog.shell, SWT.PUSH);
      button.setText(BaseMessages.getString(PKG, i18nKey));
      if (listener != null) {
