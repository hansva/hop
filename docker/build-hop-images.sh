...
#                                 Options: client, client-minimal, web, web-beam, dataflow, all
...
# Maps image type to image name
# Parse stage name and return base image name
...
build_images() {
    ...
    if [[ "$IMAGES" == "all" ]]; then
        ...
        images_to_build=("client" "web" "web-beam" "dataflow")
    else
...
print_summary() {
    ...
    if [[ "$IMAGES" == "all" ]]; then
        ...
        images_to_check=("client" "web" "web-beam" "dataflow")
    else
    ...
    for image in "${images_to_check[@]}"; do
        ...
        if [[ -n "$image_name" ]]; then
            # Add variant suffix to version if it exists
            local version_tag="$VERSION"
            if [ -n "$variant_suffix" ]; then
                version_tag="${VERSION}-${variant_suffix}"
            fi
            ...
            if [ -n "$REGISTRY" ]; then
                local full_name="$REGISTRY/$image_name:$version_tag"
            else
                local full_name="$image_name:$version_tag"
            fi
            echo "  - $full_name"
        fi
    done
